...
bool CLUT::addBitmap2(FIBITMAP* bitmap, Segment& segment) {
    unsigned int width = FreeImage_GetWidth(bitmap);
    unsigned int height = FreeImage_GetHeight(bitmap);
    for (unsigned int y = 0; y < height; y++) {
        for (unsigned int x = 0; x < width; x++) {
    ...
    this->indexImage(bitmap, segment);
...
unsigned char* CLUT::toBuffer(size_t* sizeOut) {
    /*if (this->colors.size() != this->size) {
        printf("%d : %d", this->colors.size(), this->size);
        printf("Error\n");
    }*/
...
void CLUT::indexImage(FIBITMAP* image, Segment& segment) {
    std::vector<int> ind;
    ...
    this->indexed.push_back({ ind, segment });
}